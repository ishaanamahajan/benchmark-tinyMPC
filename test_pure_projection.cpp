...
int main() {
    ...
    // ========== TEST 2: 4x4 State Consistency Matrix ==========
    std::cout << "\n🧪 TEST 2: 4x4 State Consistency Matrix" << std::endl;
    std::cout << "----------------------------------------" << std::endl;
    ...
    // ========== TEST 4: Obstacle Constraint Test ==========
    std::cout << "\n🧪 TEST 4: Obstacle Constraint Validation" << std::endl;
    std::cout << "-------------------------------------------" << std::endl;
    
    Vector4d x_violating(-5.0, 0.0, 0.0, 0.0);  // Right at obstacle center
    Vector2d x_obs(-5.0, 0.0);
    double r_obs = 2.0;
    
    // Build moment matrix for violating position
    Matrix<tinytype, 5, 5> test4;
    test4.setZero();
    test4(0, 0) = 1.0;
    test4.block<1, 4>(0, 1) = x_violating.transpose();
    test4.block<4, 1>(1, 0) = x_violating;
    test4.block<4, 4>(1, 1) = x_violating * x_violating.transpose();
    
    // Check obstacle constraint BEFORE projection
    Matrix4d XX_orig = test4.block<4, 4>(1, 1);
    double constraint_orig = XX_orig(0,0) + XX_orig(1,1) - 2.0*(x_obs(0)*x_violating(0) + x_obs(1)*x_violating(1)) + x_obs.dot(x_obs) - r_obs*r_obs;
    
    std::cout << "Before projection:" << std::endl;
    std::cout << "  Position: [" << x_violating.head<2>().transpose() << "]" << std::endl;
    std::cout << "  Distance to obstacle: " << (x_violating.head<2>() - x_obs).norm() << std::endl;
    std::cout << "  Obstacle constraint: " << constraint_orig << (constraint_orig >= 0 ? " ✅" : " ❌") << std::endl;
    
    // Project the matrix
    Matrix<tinytype, 5, 5> test4_proj = project_psd<5>(test4);
    
    // Extract projected state
    Vector4d x_projected = test4_proj.block<4, 1>(1, 0);
    Matrix4d XX_proj = test4_proj.block<4, 4>(1, 1);
    double constraint_proj = XX_proj(0,0) + XX_proj(1,1) - 2.0*(x_obs(0)*x_projected(0) + x_obs(1)*x_projected(1)) + x_obs.dot(x_obs) - r_obs*r_obs;
    
    std::cout << "After projection:" << std::endl;
    std::cout << "  Position: [" << x_projected.head<2>().transpose() << "]" << std::endl;
    std::cout << "  Distance to obstacle: " << (x_projected.head<2>() - x_obs).norm() << std::endl;
    std::cout << "  Obstacle constraint: " << constraint_proj << (constraint_proj >= 0 ? " ✅" : " ❌") << std::endl;
