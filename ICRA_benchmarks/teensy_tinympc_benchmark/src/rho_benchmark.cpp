...
//     float x_k[BENCH_NX];
//     float u_k[BENCH_NU];
//     float z_k[BENCH_NX + BENCH_NU]; // For slack variables
...
//     for(int i = 0; i < BENCH_NU; i++) {
//         z_k[BENCH_NX + i] = u_k[i]; // Typically slack variables match control inputs
//     }
    
//     // Pre-allocate temporary buffers (stack allocation is faster than heap)
//     float temp_Ax[BENCH_NX + BENCH_NU]; 
//     float temp_Px[BENCH_NX + BENCH_NU];
//     float temp_ATy[BENCH_NX + BENCH_NU];
    
//     // Compute A*x directly
...
//     // Compute q norm directly (assuming zero reference for simplicity)
...
void benchmark_rho_adaptation(
    ...
    // Verify we're using 12 states (BENCH_NX must be 12)
    if (BENCH_NX != 12) {
        result->time_us = 0;
        result->final_rho = adapter->rho_base;
        return;
    }
    
    // Initialize before timing starts
    initialize_benchmark_cache();

    adapter->clip = true;
    
    // Start timing with more precise measurement
    unsigned long start_time = micros();
    
    // Copy input state (use stack allocation for efficiency)
    float x_k[BENCH_NX];
    float u_k[BENCH_NU];
    ...
    // Update cache using Taylor expansion
    update_cache_taylor(new_rho, rho);
    
    // Update the adapter's current rho value
    adapter->rho_base = new_rho;
    
    // Calculate elapsed time with minimum time check
    unsigned long elapsed = micros() - start_time;
    if (elapsed == 0) {
        delayMicroseconds(10);  // Small delay
        elapsed = micros() - start_time;
        if (elapsed == 0) elapsed = 1;  // Minimum value if still zero
    }
    
    // Store results
    result->time_us = elapsed;
    result->initial_rho = rho;
    result->final_rho = new_rho;
    result->pri_res = pri_res;
    result->dual_res = dual_res;
    result->pri_norm = pri_norm;
    ...
    #ifdef DEBUG_RHO
    Serial.print("pri_res: "); Serial.println(pri_res);
    Serial.print("dual_res: "); Serial.println(dual_res);
    Serial.print("pri_norm: "); Serial.println(pri_norm);
    Serial.print("dual_norm: "); Serial.println(dual_norm);
    Serial.print("ratio: "); Serial.println(ratio);
    Serial.print("new_rho: "); Serial.println(new_rho);
    Serial.print("time_us: "); Serial.println(result->time_us);
    #endif
}
