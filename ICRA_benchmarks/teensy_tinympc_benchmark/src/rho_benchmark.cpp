...
#include <Arduino.h>
#include "problem_data/problem_100.hpp"
...
// Helper function to copy data from PROGMEM to a matrix
template<typename T>
void copy_from_progmem(const T* src, T* dest, size_t size) {
    for (size_t i = 0; i < size; i++) {
        dest[i] = pgm_read_float(&src[i]);
    }
}

// void initialize_matrices() {
//     // Initialize Kinf from PROGMEM
//     copy_from_progmem(Kinf_data, &Kinf[0][0], BENCH_NU * BENCH_NX);
    
//     // Initialize Pinf from PROGMEM
//     copy_from_progmem(Pinf_data, &Pinf[0][0], BENCH_NX * BENCH_NX);
    
//     // Initialize C1 from PROGMEM (using Quu_inv_data)
//     copy_from_progmem(Quu_inv_data, &C1[0][0], BENCH_NU * BENCH_NU);
    
//     // Initialize C2 from PROGMEM (using AmBKt_data)
//     copy_from_progmem(AmBKt_data, &C2[0][0], BENCH_NX * BENCH_NX);
// }



// Function to compute sensitivity updates
void update_matrices_with_sensitivity(float delta_rho) {
    // Temporary arrays for sensitivity updates
    float dKinf_update[BENCH_NU * BENCH_NX];
    float dPinf_update[BENCH_NX * BENCH_NX];
    float dC1_update[BENCH_NU * BENCH_NU];
    float dC2_update[BENCH_NX * BENCH_NX];
    
    // Copy sensitivity data from PROGMEM
    copy_from_progmem(dKinf_drho_data, dKinf_update, BENCH_NU * BENCH_NX);
    copy_from_progmem(dPinf_drho_data, dPinf_update, BENCH_NX * BENCH_NX);
    copy_from_progmem(dC1_drho_data, dC1_update, BENCH_NU * BENCH_NU);
    copy_from_progmem(dC2_drho_data, dC2_update, BENCH_NX * BENCH_NX);
    
    // Apply sensitivity updates
    for (int i = 0; i < BENCH_NU * BENCH_NX; i++) {
        (&Kinf[0][0])[i] += delta_rho * dKinf_update[i];
    }
    
    for (int i = 0; i < BENCH_NX * BENCH_NX; i++) {
        (&Pinf[0][0])[i] += delta_rho * dPinf_update[i];
    }
    
    for (int i = 0; i < BENCH_NU * BENCH_NU; i++) {
        (&C1[0][0])[i] += delta_rho * dC1_update[i];
    }
    
    for (int i = 0; i < BENCH_NX * BENCH_NX; i++) {
        (&C2[0][0])[i] += delta_rho * dC2_update[i];
    }
}
...
void initialize_benchmark_cache() {
    // Initialize matrices from PROGMEM data
    copy_from_progmem(Kinf_data, &Kinf[0][0], BENCH_NU * BENCH_NX);
    copy_from_progmem(Pinf_data, &Pinf[0][0], BENCH_NX * BENCH_NX);
    copy_from_progmem(Quu_inv_data, &C1[0][0], BENCH_NU * BENCH_NU);
    copy_from_progmem(AmBKt_data, &C2[0][0], BENCH_NX * BENCH_NX);
...
void update_cache_taylor(float new_rho, float old_rho) {
    ...
    for(int i = 0; i < BENCH_NU; i++) {
        for(int j = 0; j < BENCH_NX; j++) {
            Kinf[i][j] += delta_rho * dKinf_update[i * BENCH_NX + j];
        }
    }
    
    for(int i = 0; i < BENCH_NX; i++) {
        for(int j = 0; j < BENCH_NX; j++) {
            Pinf[i][j] += delta_rho * dPinf_update[i * BENCH_NX + j];
        }
    }
    
    for(int i = 0; i < BENCH_NU; i++) {
        for(int j = 0; j < BENCH_NU; j++) {
            C1[i][j] += delta_rho * dC1_update[i * BENCH_NU + j];
        }
    }
    
    for(int i = 0; i < BENCH_NX; i++) {
        for(int j = 0; j < BENCH_NX; j++) {
            C2[i][j] += delta_rho * dC2_update[i * BENCH_NX + j];
        }
    }
}
