...
int main(){
  ...
  // Record RAM just before solve - this is critical timing
  uint32_t pre_solve_ram = freeRam();
  Serial.print("# RAM just before first solve: ");
  Serial.println(pre_solve_ram);
  ...
  // Record RAM immediately after solve
  post_solve_ram = freeRam();
  
  // If we get here, success!
  Serial.println("# ✓ SUCCESS: First solve completed!");
  Serial.print("# RAM immediately after first solve: ");
  Serial.println(post_solve_ram);
  
  // Calculate allocation during solve specifically
  if (pre_solve_ram > post_solve_ram) {
    uint32_t solve_alloc = pre_solve_ram - post_solve_ram;
    Serial.print("# SOLVE-SPECIFIC ALLOCATION: ");
    Serial.print(solve_alloc);
    Serial.print(" bytes (");
    Serial.print(solve_alloc / 1024.0, 2);
    Serial.println(" KB)");
  }
  ...
  Serial.println("# ========================================");
  Serial.println("# NOTE: If ECOS and SCS show identical allocation for same N,");
  Serial.println("# this could indicate:");
  Serial.println("# 1. Similar QP solver memory patterns");
  Serial.println("# 2. Static allocation rather than dynamic");
  Serial.println("# 3. Measurement timing issues");
  Serial.println("# ========================================");
